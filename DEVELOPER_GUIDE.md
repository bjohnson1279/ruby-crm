...
## 8. Testing Suite & Practices

The application uses **Minitest** to verify domains logic, model attributes, and REST endpoint behaviors.

### Test Structure
Tests are organized by their architectural function:
* **Model Tests** ([test/models/](file:///c:/Users/johns/DEV/ruby-crm/test/models/)): Test validations, custom scopes, and database associations.
* **Service Tests** ([test/services/](file:///c:/Users/johns/DEV/ruby-crm/test/services/)): Verify business logic commands, audit logs integration, and transaction boundary success.
* **Controller Tests** ([test/controllers/](file:///c:/Users/johns/DEV/ruby-crm/test/controllers/)): Integration and API tests verifying endpoints responses, headers authentication, and tenant data isolation.

### Testing Helpers & Context Management
All tests inherit from `ActiveSupport::TestCase` in [test/test_helper.rb](file:///c:/Users/johns/DEV/ruby-crm/test/test_helper.rb):
1. **Thread Context Isolation**: The global `setup` and `teardown` blocks invoke `Current.reset` to prevent `Current.firm` or `Current.user` leakages across parallel workers.
2. **Context Setup**: `setup_default_tenant` instantiates a default firm and advisor and configures the `Current` context.
3. **HTTP Scoping**: `authenticated_headers(firm, user)` returns mock credentials for integration test requests.
